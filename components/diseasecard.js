...
export default function DiseaseCard({}) {
    ...
    const callApi = token =>{
        const url = apiConfig.baseURL + 'api/diseases?per_page=15&page='+ pageCurrent 
    ...
    const onShare = async (txt) => {
        try {
            const result = await Share.share({
                message:apiConfig.baseURL+'/diseases/'+txt,
            });
        } catch (error) {
            alert(error.message);
        }
                ...
                <FlatList
                    ...
                    renderItem={({item,index}) =>
                        <View style={styles.card}>
                            <TouchableOpacity 
                            style={styles.main}
                            onPress={()=>openLink(apiConfig.baseURL + '/web_view/diseases/' + item.id)}>
